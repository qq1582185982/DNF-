...
private:
    ...
    void announce_peer_offers_for_session(const shared_ptr<PacketTunnelSession>& session,
        ...
        for (size_t i = 0; i < peers.size(); ++i) {
            ...
            Logger::debug("[" + server_name + "|IP Tunnel] announce udp peer offers " +
                          local_scope_label + " <-> " + peer_scope_label +
                          " local_candidates=" + to_string(sent_local_candidates) +
                          " peer_candidates=" + to_string(sent_peer_candidates));
        }
    ...
    void announce_tcp_peer_offers_for_session(const shared_ptr<PacketTunnelSession>& session,
        ...
        for (size_t i = 0; i < peers.size(); ++i) {
            ...
            Logger::debug("[" + server_name + "|IP Tunnel] announce tcp peer offers " +
            ...
            if (n <= 0) {
                ...
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    ...
                    for (size_t i = 0; i < expired_tcp_sessions.size(); ++i) {
                        Logger::info("[" + server_name + "|IP Tunnel] drop invalid TCP session: virtual_ip=" +
                                     ipv4_be_to_string(expired_tcp_sessions[i].session->virtual_ip_be) +
                                     " endpoint=" + expired_tcp_sessions[i].session->client_str +
                                     " reason=" + expired_tcp_sessions[i].reason);
                    }
                    continue;
                }
                if (errno == EINTR) {
    ...
    void handle_packet_tunnel(int client_fd, const string& client_str, const string& session_uuid) {
        ...
        try {
            ...
            if (!recv_all_exact(client_fd, &next_handshake_byte, 1)) {
                Logger::error("[IP Tunnel|" + session_uuid + "] TCP handshake extension header read failed");
                close(client_fd);
                return;
            }
            if (next_handshake_byte == packet_tunnel::kProtocolVersion) {
                ...
                if (!recv_all_exact(client_fd, tail + 1, sizeof(tail) - 1)) {
                    Logger::error("[IP Tunnel|" + session_uuid + "] legacy TCP handshake tail read failed");
                    close(client_fd);
                    return;
                }
                tail_loaded = true;
                ...
                if (client_id_len > 0) {
                    ...
                    if (!recv_all_exact(client_fd,
                                        ...
                                        client_id_len)) {
                        Logger::error("[IP Tunnel|" + session_uuid + "] TCP handshake client_id read failed");
                        close(client_fd);
                        return;
                    }
            ...
            Logger::debug("[IP Tunnel|" + session_uuid + "] 握手参数: version=" + to_string((int)version) +
                         ...
                         ", flags=" + to_string((int)flags) +
                         (client_id_short.empty() ? "" : ", client_id=" + client_id_short));
