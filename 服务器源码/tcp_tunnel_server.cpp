    ...
    static void rotate_log_file() {
        ...
        if (log_file.is_open()) {
            ...
            ss << put_time(gmtime(&beijing_time), "%Y-%m-%d %H:%M:%S")
               ...
               << " [信息] 日志文件轮转: 关闭旧日志，日期从 " << current_date << " 切换到 " << new_date;
...
public:
    ...
    static void init(const string& filename) {
        ...
        if (log_file.is_open()) {
            ...
            ss << put_time(gmtime(&beijing_time), "%Y-%m-%d %H:%M:%S")
               << "." << setfill('0') << setw(3) << ms.count()
               << " [信息] 日志文件已初始化(北京时间, 自动按天分割): " << dated_filename;
...
private:
    static void log(const string& level, const string& msg) {
        ...
        if (level == "DEBUG") display_level = "调试";
        else if (level == "INFO") display_level = "信息";
        else if (level == "WARN") display_level = "警告";
        else if (level == "ERROR") display_level = "错误";
...
static bool parse_tcp_direct_candidate_advertise_frame(const uint8_t* payload,
                                                       size_t payload_len,
                                                       TcpDirectCandidate* out_candidate) {
    if (payload == nullptr || out_candidate == nullptr ||
        payload_len != packet_tunnel::kTcpDirectCandidateAdvertisePayloadSize) {
        return false;
    }
    const uint8_t family = payload[0];
    const uint16_t port = packet_tunnel::read_u16_be(payload + 2);
    if (port == 0 ||
    ...
    out_candidate->endpoint_family = family;
    out_candidate->endpoint_port = port;
    memset(out_candidate->endpoint_addr, 0, sizeof(out_candidate->endpoint_addr));
    memcpy(out_candidate->endpoint_addr,
           payload + 4,
           family == packet_tunnel::kPeerEndpointFamilyIpv4 ? 4 : 16);
    return true;
}

static bool parse_udp_direct_candidate_advertise_frame(const uint8_t* payload,
                                                       size_t payload_len,
                                                       TcpDirectCandidate* out_candidate) {
    if (payload_len != packet_tunnel::kUdpDirectCandidateAdvertisePayloadSize) {
        return false;
    }
...
static bool encode_tcp_candidate_peer_offer_payload(uint32_t peer_virtual_ip_be,
    ...
    payload[12] = candidate.endpoint_family;
    packet_tunnel::write_u16_be(payload.data() + 14, candidate.endpoint_port);
...
private:
    ...
    void learn_gateway_udp_port_owner(const shared_ptr<PacketTunnelSession>& session,
        ...
        if (owner_changed) {
            Logger::debug("[IP Tunnel|" + session->session_uuid + "] 已更新网关UDP端口归属 端口=" +
    ...
    bool relay_gateway_udp_to_peer(const shared_ptr<PacketTunnelSession>& sender_session,
        ...
        if (!try_resolve_gateway_udp_peer_target(sender_session,
            ...
            Logger::info("[IP Tunnel|" + sender_session->session_uuid +
                         ...
                         to_string(dst_port) + " 解析方式=" + resolution +
        ...
        if (!rewrite_client_bound_udp_destination_ip(&rewritten_packet,
            ...
            Logger::warning("[IP Tunnel|" + sender_session->session_uuid +
                            ...
                            ipv4_be_to_string(src_ip_be) + ":" + to_string(src_port) +
                            " 目标=" + ipv4_be_to_string(original_dst_ip_be) + ":" +
                            to_string(dst_port) + " 对端=" +
                            describe_scoped_virtual_ip(target_session));
            return true;
        }

        Logger::info("[IP Tunnel|" + sender_session->session_uuid + "] 中转网关UDP 源=" +
                     ipv4_be_to_string(src_ip_be) + ":" + to_string(src_port) +
    ...
    void log_expired_peer_coord_states(PeerCoord* peer_coord, const string& server_name) {
        ...
        for (size_t i = 0; i < changed.size(); ++i) {
            Logger::debug("[" + server_name + "|IP Tunnel] 对等端状态切换 " +
        ...
        if (!changed.empty()) {
            ...
            Logger::debug("[" + server_name + "|IP Tunnel] 对等端快照: " +
    ...
    bool send_packet_tunnel_datagram(const shared_ptr<PacketTunnelSession>& session,
        ...
        if (session->use_udp) {
            ...
            if (send_elapsed_ms >= kSlowPacketTunnelFrameSendWarnMs) {
                Logger::warning("[" + server_name + "|IP Tunnel|" + session->session_uuid +
                                "] 帧发送耗时偏长 elapsed_ms=" + to_string(send_elapsed_ms) +
                                " 帧类型=" + frame_label +
                                " 帧数=" + to_string(frame_count) +
                                " 模式=UDP 负载长度=" + to_string(payload_len) +
                                " 目标=" + describe_scoped_virtual_ip(session));
            }
            return true;
        }
        ...
        if (send_elapsed_ms >= kSlowPacketTunnelFrameSendWarnMs) {
            Logger::warning("[" + server_name + "|IP Tunnel|" + session->session_uuid +
                            ...
                            " 帧类型=" + frame_label +
                            " 帧数=" + to_string(frame_count) +
                            " 模式=TCP 负载长度=" + to_string(payload_len) +
                            " 目标=" + describe_scoped_virtual_ip(session));
        }
        return true;
    }
    ...
    bool relay_virtual_peer_packet(const shared_ptr<PacketTunnelSession>& sender_session,
        ...
        uint32_t src_ip_be = 0;
        memcpy(&src_ip_be, payload + 12, sizeof(src_ip_be));
        const bool is_tcp_payload = packet_tunnel_payload_is_tcp(payload, payload_len);
        uint16_t src_port = 0;
        uint16_t dst_port = 0;
        ...
        shared_ptr<PacketTunnelSession> peer_session;
        shared_ptr<PacketTunnelSession> target_session;
        {
            lock_guard<mutex> lock(packet_tunnel_mutex);
    ...
    vector<PacketTunnelSessionRemoval> cleanup_packet_tunnel_tcp_sessions(uint64_t now_ms) {
        ...
        for (map<string, shared_ptr<PacketTunnelSession>>::iterator it = packet_tunnel_tcp_sessions.begin();
            ...
            if (session) {
                removed.push_back(PacketTunnelSessionRemoval(session, remove_reason));
                erase_packet_tunnel_tcp_session_locked(removed_session);
            }
        }
    ...
    void announce_peer_offers_for_session(const shared_ptr<PacketTunnelSession>& session,
        ...
        if (is_peer_direct_excluded_session(session)) {
            Logger::debug("[" + server_name + "|IP Tunnel] 跳过对等端提议，当前会话被排除出直连: " +
        ...
        if (!session_has_active_lease(session, nullptr, &session_lease_error)) {
            Logger::debug("[" + server_name + "|IP Tunnel] 跳过对等端提议，会话没有有效租约: " +
        ...
        if (local_candidates.empty()) {
            Logger::warning("[" + server_name + "|IP Tunnel] 编码对等端提议失败: " +
                            local_scope_label);
            return;
        }
        ...
        for (size_t i = 0; i < peers.size(); ++i) {
            ...
            if (!session_allows_peer_direct(session) || !session_allows_peer_direct(peer)) {
                ...
                Logger::debug("[" + server_name + "|IP Tunnel] 跳过对等端提议，双方仅允许中转: " +
            ...
            Logger::debug("[" + server_name + "|IP Tunnel] 已广播UDP对等端提议 " +
                          local_scope_label + " <-> " + peer_scope_label +
                          " 本地候选=" + to_string(sent_local_candidates) +
                          " 对端候选=" + to_string(sent_peer_candidates));
        }
    ...
    void announce_tcp_peer_offers_for_session(const shared_ptr<PacketTunnelSession>& session,
        ...
        if (local_candidates.empty()) {
            Logger::warning("[" + server_name + "|IP Tunnel] 编码TCP对等端提议失败: " +
                            describe_scoped_virtual_ip(session));
            return;
        }
        ...
        for (size_t i = 0; i < peers.size(); ++i) {
            const shared_ptr<PacketTunnelSession>& peer = peers[i];
            const vector<TcpDirectCandidate> peer_candidates =
                build_tcp_direct_candidates_for_session(peer);
            if (peer_candidates.empty()) {
                continue;
            }
            ...
            Logger::debug("[" + server_name + "|IP Tunnel] 已广播TCP对等端提议 " +
    ...
    bool route_peer_signal_frame(const shared_ptr<PacketTunnelSession>& sender_session,
        ...
        if (!session_allows_peer_direct(sender_session) ||
            ...
            Logger::debug("[IP Tunnel|" + sender_session->session_uuid +
                          "] 抑制" + packet_tunnel_frame_name(frame_type) +
    ...
    void packet_tunnel_tun_loop() {
        ...
        while (running && tun_manager.IsActive()) {
            ...
            if (!session) {
                Logger::debug("[" + server_name + "|IP Tunnel] 未找到目标虚拟IP对应会话: " +
                              (scoped_server_key.empty()
                                   ? ipv4_be_to_string(dst_ip_be)
                                   : describe_scoped_virtual_ip(scoped_server_key, dst_ip_be)) +
                              " 源=" + ipv4_be_to_string(src_ip_be) +
                              " 协议=" + to_string((int)ipv4_protocol(packet)));
                continue;
            }
            ...
            if (protocol == IPPROTO_UDP &&
                ...
                if (rewrite_client_bound_udp_source_ip(&packet, gateway_ip_be, server_virtual_ip_be)) {
                    Logger::debug("[IP Tunnel|" + session->session_uuid + "] 已改写UDP回复源地址 " +
...
public:
    ...
    bool start() {
        ...
        for (vector<pair<string, string> >::const_iterator it = config.local_node_server_keys.begin();
            ...
            if (!parse_ipv4_be(it->first, &local_node_ip_be)) {
                Logger::warning("[" + server_name + "] 本地节点server_key映射中的virtual_ip无效: " +
                                it->first);
                continue;
            }
        ...
        for (vector<pair<string, string> >::const_iterator it = config.remote_node_server_keys.begin();
            ...
            if (!parse_ipv4_be(it->first, &remote_node_ip_be)) {
                Logger::warning("[" + server_name + "] 远端节点server_key映射中的virtual_ip无效: " +
                                it->first);
                continue;
            }
        ...
        if (tun_manager.Setup(tun_config, &tun_error)) {
            ...
            if (!tun_config.server_virtual_ip.empty()) {
                tun_log += " 服务端IP=" + tun_config.server_virtual_ip;
            }
            Logger::info(tun_log);
    ...
    void stop() {
        ...
        vector<int> packet_fds;
        {
            ...
            for (auto it = packet_tunnel_tcp_sessions.begin(); it != packet_tunnel_tcp_sessions.end(); ++it) {
                it->second->active = false;
                packet_fds.push_back(it->second->client_fd);
            }
...
private:
    void packet_tunnel_udp_loop() {
        while (running && udp_fd >= 0) {
            ...
            if (n <= 0) {
                ...
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    ...
                    for (size_t i = 0; i < expired_tcp_sessions.size(); ++i) {
                        Logger::info("[" + server_name + "|IP Tunnel] 丢弃无效TCP会话: 虚拟IP=" +
                ...
                if (errno != EBADF && errno != EINVAL) {
                    Logger::warning("[" + server_name + "|IP Tunnel] recvfrom失败: " + string(strerror(errno)));
                }
                break;
            }
                ...
                if (conn_id == packet_tunnel::kHandshakeConnId &&
                    ...
                    *(uint16_t*)(ack + 2) = htons(mtu);
                    ...
                    if (ack_sent != (int)sizeof(ack)) {
                        Logger::warning("[IP Tunnel|" + session_uuid + "] 发送UDP握手确认失败");
                        continue;
                    }
            ...
            if (frame_type == packet_tunnel::kFrameUdpDirectCandidateAdvertise) {
                ...
                Logger::info("[IP Tunnel|" + session->session_uuid + "] UDP直连候选上报: 虚拟IP=" +
            ...
            if (is_peer_control_frame(frame_type)) {
                if (frame_type == packet_tunnel::kFramePeerOffer) {
                    ...
                    peer_coord_.ObservePeerFrame(build_scoped_virtual_ip_key(session->server_key,
                                                 ...
                                                 PeerEndpointState::OfferPending);
                    Logger::debug("[IP Tunnel|" + session->session_uuid + "] 对等控制 " +
                                  packet_tunnel_frame_name(frame_type) +
                                  ": 对端=" + ipv4_be_to_string(offer.peer_virtual_ip_be) +
                                  " 版本=" + to_string(offer.endpoint_version) +
                                  " 地址族=" + to_string((int)offer.endpoint_family) +
                                  " 端口=" + to_string(offer.endpoint_port));
                    continue;
                }
                ...
                if (frame_type == packet_tunnel::kFramePeerDisable) {
                    ...
                    if (!route_peer_disable_frame(session, disable)) {
                        Logger::warning("[IP Tunnel|" + session->session_uuid + "] 转发peer_disable到对端失败，对端=" +
            ...
            if (src_ip_be != session->virtual_ip_be) {
                Logger::warning("[IP Tunnel|" + session->session_uuid + "] 源IP不匹配: src=" +
            ...
            if (!route_to_local_gateway && !route_to_local_node) {
                Logger::debug("[IP Tunnel|" + session->session_uuid + "] 目标没有对端或本地路由: dst=" +
    ...
    void handle_packet_tunnel(int client_fd, const string& client_str, const string& session_uuid) {
        ...
        try {
            ...
            if (!tail_loaded && !recv_all_exact(client_fd, tail, sizeof(tail))) {
            ...
            *(uint16_t*)(ack + 2) = htons(mtu);
            ...
            Logger::info("[IP Tunnel|" + session_uuid + "] TCP会话已建立: 客户端=" + client_str +
                         ", 虚拟IP=" + describe_scoped_virtual_ip(resolved_server_key, virtual_ip_be) +
                         ", 直连=" +
                         string(session->allow_peer_direct ? "已启用" :
                                (session->is_remote_linux_node ? "仅中转(远端Linux节点)"
            ...
            while (running && session->active) {
                ...
                if (frame_type == packet_tunnel::kFrameTcpDirectCandidateAdvertise) {
                    TcpDirectCandidate candidate;
                ...
                if (is_peer_control_frame(frame_type)) {
                    if (frame_type == packet_tunnel::kFramePeerOffer) {
                        ...
                        peer_coord_.ObservePeerFrame(build_scoped_virtual_ip_key(session->server_key,
                                                     ...
                                                     offer.endpoint_version,
                                                     PeerEndpointState::OfferPending);
                        Logger::debug("[IP Tunnel|" + session_uuid + "] 对等控制 " +
                                      packet_tunnel_frame_name(frame_type) +
                                      ": 对端=" + ipv4_be_to_string(offer.peer_virtual_ip_be) +
                                      " 版本=" + to_string(offer.endpoint_version) +
                                      " 地址族=" + to_string((int)offer.endpoint_family) +
                ...
                if (frame_type == packet_tunnel::kFrameIpv4Packet) {
                    ...
                    if (src_ip_be != virtual_ip_be) {
                        Logger::warning("[IP Tunnel|" + session_uuid + "] 源IP不匹配: 源=" +
                    ...
                    if (!dst_is_game_server && !dst_is_virtual_peer) {
                        Logger::debug("[IP Tunnel|" + session_uuid + "] 忽略非隧道路由: dst=" +
                                      ipv4_be_to_string(dst_ip_be));
                        continue;
                    }
                    ...
                    if (!route_to_local_gateway && !route_to_local_node) {
                        Logger::debug("[IP Tunnel|" + session_uuid + "] 目标没有对端或本地路由: dst=" +
                                      describe_scoped_virtual_ip(session->server_key, dst_ip_be));
                        continue;
                    }
                    ...
                    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_UDP ||
                        ...
                        Logger::debug("[IP Tunnel|" + session_uuid + "] 客户端->TUN 源=" +
        ...
        if (session) {
            lock_guard<mutex> lock(packet_tunnel_mutex);
            erase_packet_tunnel_tcp_session_locked(session);
        }
