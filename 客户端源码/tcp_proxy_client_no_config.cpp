 ...
 * - 预期效果: 游戏服务器无法通过窗口行为检测代理，连接可长期稳定(测试>9.8小时)
...
public:
    TCPConnection(int id, const string& sip, uint16_t sport,
        ...
        // v12.3.11: 关键修复 - data_window动态跟随client_window
        //          之前固定1024导致游戏服务器检测到窗口从不变化，判定为代理特征
        //          抓包分析显示真实游戏客户端窗口动态变化(200-1000范围)
        //          现改为实时同步游戏真实窗口，消除代理特征，防止8-10分钟断线
        // 设计：- client_window：从游戏包中提取的真实窗口值
        //       - data_window：同步client_window，向游戏服务器通告相同窗口
        //       - 窗口变化包括: 握手65535 → 数据传输200-1000动态调整 → 流控时0
